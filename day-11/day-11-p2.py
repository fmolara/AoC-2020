...
def IsValidCoo( image, row, col ):
    return 0 <= row < image.shape[0] and 0 <= col < image.shape[1]

def GetMap( image, row, col ):
    assert IsValidCoo( image, row, col )
...
def CountItems( image, pos, item ):
    ...
    for row, col in directions:
        for radius in range(1, max(image.shape)):
            coo = ( pos[0]+radius*row
                  , pos[1]+radius*col )
            if not IsValidCoo( image, coo[0], coo[1] ):
                break
            if GetMap( image, coo[0], coo[1] ) == '.':
                continue
            cnt += 1 if GetMap( image, coo[0], coo[1] ) == item else 0
            break
    return cnt
...
def TurnSeats( image_0 ):
    ...
    for row in range(image_0.shape[0]):
        for col in range(image_0.shape[1]):
            ...
            elif GetMap( image_0, row, col ) == '#' and CountItems( image_0, (row, col), '#' ) >= 5:
